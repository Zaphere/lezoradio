...
export async function fetchNewsItems(region?: string, category?: string): Promise<any[]> {
  ...
  try {
    ...
    console.warn(`[fetchNewsItems] Proxy returned HTTP ${response.status}, falling back to direct queries`);
  } catch (err) {
  ...
  const newsItems = await _silentFetch(async () => {
    let q = supabase.from('news_items').select('id, feed_id, title, description, content, url, region, category, published_at, ingested_at, is_processed').gte('ingested_at', retentionCutoff()).order('ingested_at', { ascending: false }).limit(50);
    if (region) {
