  ...
  return (
    <div className="glass rounded-2xl p-4 space-y-3">
      ...
      <div className="flex items-center gap-3 text-[10px] text-text-secondary">
        <span className="px-2 py-0.5 rounded-full bg-white/10">{item.source}</span>
        {item.country && (
          <span className="px-2 py-0.5 rounded-full bg-white/10">{item.country}</span>
        )}
        <span className="px-2 py-0.5 rounded-full bg-white/10 capitalize">{item.type}</span>
